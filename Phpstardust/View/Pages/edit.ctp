<h1><?php echo __d('phpstardust', 'Edit Page'); ?></h1>
...
if ($this->data['Page']['oldimage']!="") {
	echo $this->Html->link(
		__d('phpstardust', 'View image'),
		Configure::read('Psd.url') .'/files/uploads/' .$this->data['Page']['image'],
		array('target' => '_blank')
	);
	echo '<br><br>';
	echo $this->Html->link(
		__d('phpstardust', 'Delete image'), 
		array('action' => 'deleteImage', $this->data['Page']['id']),
